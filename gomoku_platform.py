...
def contest_once(n, ai_black, ai_white):
	...
	colors = [colorama.Fore.BLUE + colorama.Style.DIM, colorama.Fore.RED + colorama.Style.DIM]
	colors_bg = [colorama.Back.BLUE, colorama.Back.RED]
	players = [
...
class Grid:
	...
	def str(self, bg=''):
		return '%s%s%s%3d%s' % (self.color, colorama.Style.BRIGHT if self.active else '', bg, self.step, colorama.Style.RESET_ALL)
